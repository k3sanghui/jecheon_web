...
'''
@bp.route('/update/<int:seq_user>', methods=('GET', 'POST'))
def user_update(seq_user):
    form = UserCreateForm()
    if request.method == 'POST' and form.validate_on_submit():
        id_user = form.id_user.data
        ds_name = form.ds_name.data
        ...
        sql = "update tapp_memb010 set ds_name" \
              "ds_password = %s," \
              "ds_tel = %s," \
              "ds_email = %s," \
              "no_level = %s," \
              "yn_emailcertify = %s"
        sql_common = (ds_name, ds_password, ds_tel, ds_email, no_level, yn_emailcertify)
...
def login():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        sql = "SELECT seq_user, id_user, ds_password, no_level FROM tapp_memb010 where id_user = %s"
        sql_common = (id_user,)
